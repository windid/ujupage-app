<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <meta charset="UTF-8">
</head>

<body>

<div style="background-color: #eee;">
  <div class="email-container" style="background-color: #fff; margin: 10px auto; min-width: 360px; width: 540px; border-radius: 4px;">
    <div class="email-header" style="width: 100%; padding: 12px; border-bottom: 1px solid #ddd;">
      <a href="http://www.ujupage.com">
        <img src="data:image/png;base64," alt="">
      </a>
    </div>
    <div class="email-content" style="width: 100%; padding: 12px;">
      @yield('content')
    </div>
  </div>
</div>

</body>
</html>