...
describe('colorMixin.getColor', () => {
  it('should return valid color format', () => {
    ...
    const vm = new Ctor()
    expect(vm.getColor('')).to.equal('transparent')
    ...
    expect(vm.getColor('rgba(0, 0, 0, 1)')).to.equal('rgba(0, 0, 0, 1)')
    expect(vm.getColor('hsl(123,70%,30%)')).to.equal('hsl(123,70%,30%)')
    expect(vm.getColor('hsla(123, 70%, 30%, .5)')).to.equal('hsla(123, 70%, 30%, .5)')
    expect(vm.getColor('red')).to.equal('red')
  })
})
